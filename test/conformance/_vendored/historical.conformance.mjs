const assert = (cond, msg) => { if (!cond)
    throw new Error(`historical-conformance: ${msg}`); };
async function getJson(t, path) {
    const headers = t.token ? { authorization: `Bearer ${t.token}` } : {};
    const res = await fetch(`${t.baseUrl.replace(/\/+$/, '')}${path}`, { headers });
    return { status: res.status, body: await res.json().catch(() => null) };
}
const ROW_FIELDS = ['schema_version', 'minute_ts', 'symbol', 'open', 'high', 'low', 'close', 'volume', 'turnover', 'oi_total_usd', 'funding_rate', 'liq_long_usd', 'liq_short_usd', 'has_oi', 'has_funding', 'has_liquidations', 'taker_buy_volume_usd', 'taker_sell_volume_usd', 'has_taker_flow'];
...
export async function runHistoricalConformance(t, opts = {}) {
    // discover
    ...
    assert(d.body.resources?.some((r) => r.name === 'rows' && r.availability === 'available'), 'discover missing rows resource');
    ...
    for (const f of ROW_FIELDS)
        assert(f in full.body.items[0], `rows[0] missing field ${f}`);
    ...
    if (opts.goldenRows && opts.goldenRows.length > 0) {
        const got = full.body.items.map((r) => normalizeRow(r)).join('\n');
        const want = opts.goldenRows.map((r) => normalizeRow(r)).join('\n');
