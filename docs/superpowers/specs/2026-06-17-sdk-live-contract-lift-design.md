...
- **New conformance fixture** `packages/sdk/conformance/ops-read-dto.conformance.ts` (+ `tsconfig.ops-read.json`): mutual-assignability `Mutual<Sdk.X, PlatOps.X>` for every lifted type, compiled `tsc --noEmit` against `dist/src/operations/dto.js` — the exact pattern of `paper-candidate.conformance.ts` (036). A new `verify_sdk_ops_read_conformance.mjs` (modeled on `verify_036_type_conformance.mjs`) runs it under a **dedicated** `gates:sdk-ops-read` aggregate (NOT folded into `gates:033`, to keep that behavioural gate's semantics clean), wired into the platform's full-suite check chain.
